...
public class ManuallyChosenNumbersTest {
    @ParameterizedTest
    @ValueSource(strings = {"1,2,3,4,5,6"})
    public void 로또_번호는_중복되지_않는_6개의_수로_이루어져_있다(String numbers) {
        List<Number> numberList = parseNumbers(numbers);
        ManuallyChosenNumbers manuallyChosenNumbers = new ManuallyChosenNumbers(numberList);
        Assertions.assertThat(manuallyChosenNumbers).isInstanceOf(ManuallyChosenNumbers.class);
    }
    ...
    public void 로또_번호가_중복되는_경우_예외를_던진다(String numbers) {
        List<Number> numberList = parseNumbers(numbers);
        Assertions.assertThatThrownBy(() -> new ManuallyChosenNumbers(numberList)).isInstanceOf(RuntimeException.class);
    }
